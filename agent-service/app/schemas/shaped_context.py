...
class StructureBlock:
    ...
    block_id       块 ID（由入口 symbol_key 确定性哈希，跨 revision 稳定）。
    atoms          块内原子的 symbol_key 列表。
    chunks         块内源码片段的 chunk_id 列表。
    entry_distance 到最近入口的结构距离。
    entry_label    所属入口的人类可读标签（共享依赖块为空串）。
...
class ShapedCodeContext:
    ...
    structure_blocks  按结构连通性划分的代码块（每个块对应一个入口闭包）。
    relations         本范围内原子发起的代码关系（INTERNAL/BOUNDARY/UNRESOLVED）。
    trimmed_atom_ids  因预算裁剪而未进入快照的原子 symbol_key 列表。
